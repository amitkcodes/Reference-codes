...
# 2025-05-01: Added alert for average offset exceeding 0.01 seconds in fetch_data_from_servers.
...
def fetch_data_from_servers(ntp_servers, alert_emails):
    ...
    if valid_offsets:
        ...
        # Check if average offset exceeds 0.01 seconds
        if abs(avg_offset) > 0.01:
            alert_key = "Average_Offset"
            current_time_epoch = time.time()
            if alert_key not in last_alert_time or current_time_epoch - last_alert_time[alert_key] > 3600:
                last_alert_time[alert_key] = current_time_epoch
                alert_message = (
                    f"NTP Average Offset Alert\n"
                    f"Average Offset: {avg_offset:.6f} seconds\n"
                    f"Number of Servers: {len(valid_offsets)}\n"
                    f"Time: {current_time}"
                )
                url = 'http://127.0.0.1:5000/send-sms'
                payload = {'message': alert_message, 'to_emails': alert_emails}
                headers = {'Content-Type': 'application/json'}
                try:
                    response = requests.post(url, data=json.dumps(payload), headers=headers)
                    response.raise_for_status()
                    logging.info(f"Average offset alert sent: {response.json()}")
                except requests.RequestException as e:
                    logging.error(f"Failed to send average offset alert: {e}")
